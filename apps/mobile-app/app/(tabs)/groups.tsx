...
import { View, Text, FlatList, TouchableOpacity, Dimensions, RefreshControl, ActivityIndicator } from 'react-native';
...
const ActiveGroupItem = memo(({ group, theme, fontScale, onPress }: any) => (
    <TouchableOpacity
        ...
        activeOpacity={0.85}
        style={{ backgroundColor: theme.cardSecondary, borderColor: theme.border, marginBottom: 24 }}
...
const ClosedGroupItem = memo(({ group, theme, fontScale, onPress }: any) => (
    <TouchableOpacity
        onPress={() => onPress(group.id)}
        activeOpacity={0.85}
        style={{ backgroundColor: theme.cardSecondary, borderColor: theme.border, opacity: 0.8, marginBottom: 24 }}
        className="border rounded-[36px] p-6 flex-row items-center"
    >
        <View style={{ backgroundColor: theme.glassBg }} className="w-16 h-16 rounded-[24px] items-center justify-center mr-5 border border-white/5">
...
export default function GroupListScreen() {
    ...
    const renderItem = useCallback(({ item }: any) => {
        if (activeTab === 'active') {
            return <ActiveGroupItem group={item} theme={theme} fontScale={fontScale} onPress={handleGroupPress} />;
        }
        return <ClosedGroupItem group={item} theme={theme} fontScale={fontScale} onPress={handleGroupPress} />;
    }, [activeTab, theme, fontScale, handleGroupPress]);
    ...
    ), [activeTab, theme, fontScale, user]);
    ...
    const renderFooter = useCallback(() => {
        ...
        return (
            <View
                style={{ backgroundColor: theme.cardSecondary, borderColor: theme.border, marginTop: 24 }}
                className="rounded-[40px] border p-8 items-center overflow-hidden"
            >
    ...
    }, [isLoading, activeTab, theme, fontScale]);


    const currentData = activeTab === 'active' ? activeGroups : closedGroups;
