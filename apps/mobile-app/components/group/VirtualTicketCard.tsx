    ...
    return (
        ...
        >
            <View className="flex-row justify-between items-end mb-6 mt-2 px-2">
                <View>
                    <Text style={{ color: theme.text, fontSize: 18 * fontScale }} className="font-bold tracking-tight">Detalle de la cuenta</Text>
            ...
            <View className="gap-y-4">
                {items.map((item) => (
                    <TouchableOpacity 
                        key={item.id} 
                        activeOpacity={0.7}
                        style={{ backgroundColor: theme.card, borderColor: theme.border + '20' }} 
                        className="rounded-2xl p-5 border shadow-sm"
                    >
                        <View className="flex-row justify-between items-start mb-4">
                            <View className="flex-1 pr-4">
                                <Text style={{ color: theme.text, fontSize: 16 * fontScale }} className="font-bold">{item.name}</Text>
                                <Text style={{ color: theme.textSecondary, fontSize: 14 * fontScale }} className="mt-1 opacity-70">{item.detail}</Text>
                            </View>
                            <Text style={{ color: theme.text, fontSize: 18 * fontScale }} className="font-black">${item.amount.toFixed(2)}</Text>
                        </View>
                        
                        <View className="flex-row items-center justify-between mt-2 pt-4 border-t" style={{ borderColor: theme.border + '15' }}>
                ...
                <View 
                    ...
                    className="rounded-xl p-4 flex-row justify-between items-center mt-2 border"
                >
                    <View className="flex-row items-center gap-3">
                        <View style={{ backgroundColor: theme.card }} className="w-8 h-8 rounded-full items-center justify-center shadow-sm">
